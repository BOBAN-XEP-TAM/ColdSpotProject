...
public class GameScreen implements Screen {
    ...
    public void loadActors() {
        ...
        MapLoader.buildShapes(map.getLayers().get("Obstacles").getObjects(), world, mapRenderer.getUnitScale());

        //PLAYER CONFIG
        //player = new Player(Main.mainChar_ak, new Vector2(57, 15).scl(PPM), 7);
        player = new Player(Main.mainChar_ak,  new Vector2(52.8f, 7.5f).scl(PPM), 7, world);
        player.getGun(new PlayerGun(player));

        //GRIPS CONFIG
        joystickR = new Joystick(Main.joystickImg, Main.gripImg, new Vector2((float) (Main.WIDTH - (Main.WIDTH * 0.15)), (Main.HEIGHT / 3) / 2 + (Main.HEIGHT / 3) / 4), Main.HEIGHT / 3);
        joystickL = new Joystick(Main.joystickImg, Main.gripImg, new Vector2((float)(Main.WIDTH - (Main.WIDTH * 0.85)), (Main.HEIGHT / 3) / 2 + (Main.HEIGHT / 3) / 4), Main.HEIGHT / 3);
        ...
        //FONT GENERATION
        ammo_view = new Text("fonts/ammo.ttf",(float) Main.WIDTH - 100, (float) Main.HEIGHT - 80);
        ...
        //ADDING ACTORS
        Main.stage.addActor(joystickR);
        Main.stage.addActor(joystickL);
        Main.stage.addActor(ammo_view);
    ...
    public void render(float delta) {
        changeBackgroundColor(color, Color.PURPLE, 20, delta);
    ...
    public void gameRender(SpriteBatch batch) {
        ...
        //AMMO VIEW UPDATING
        ammo_view.update(player.gun.ammo);
    ...
    private void sweepDeadBodies(Array<Body> bodiesToDestroy) {
    ...
    public void gameUpdate(){
        ...
        sweepDeadBodies(hcl.bodiesToDestroy);
...
//            enemies.get(i).handgun.update(joystickR);
        ...
        if (!player.isAlive) {
            parent.changeScreen(Main.GAMEOVER);
        }
    }
